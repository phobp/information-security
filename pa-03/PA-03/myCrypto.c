...
int BN_write_fd( const BIGNUM *bn , int fd_out ) {
    ...
    unsigned int bnBytes = 0; 
    bnBytes = BN_num_bytes( bn );
    printf("%d, bn = %s\n", bnBytes, BN_bn2hex(bn));
    write( fd_out , &bnBytes , sizeof( fd_out ) );
    unsigned char *bnWrite = malloc(bnBytes);
    BN_bn2bin( bn , bnWrite );

    if( write( fd_out , bnWrite , 64 ) < 0 ) {
...
BIGNUM *BN_read_fd( int fd_in ) {
    ...
    if (read( fd_in , &bnBytes , sizeof(bnBytes) ) <= 0) {
        printf("Failed to read size of next big number.\n");
        return NULL;
    }

    unsigned char *bnWrite = malloc(bnBytes * sizeof(unsigned char));
    if (bnWrite == NULL) {
        printf("Bad malloc. bnBytes = %ud\n", bnBytes);
        return NULL;
    }
 
    int read_fd = read( fd_in , bnWrite, bnBytes );
    BN_bin2bn( bnWrite , bnBytes, bnCpy );
    
    if ( read_fd < 0 ) {
        printf("Failed to read big number. read_fd = %d\n", read_fd);
        return NULL;
    }
...
BIGNUM *BN_myRandom( const BIGNUM *p ) {
    ...
    q_minus_one = BN_dup( p );

    BIGNUM *one = BN_new();
    BN_dec2bn( &one , "1" );
    BN_sub( q_minus_one , q_minus_one , one );
    BN_rand_range( random , q_minus_one );

    BN_sub( q_minus_one , q_minus_one , one );
    BN_mod( modRandom , random , q_minus_one , ctx );
...
void elgamalSign( const uint8_t *digest , int len , const BIGNUM *q , const BIGNUM *gen , const BIGNUM *x , BIGNUM *r , BIGNUM *s , BN_CTX *ctx ) {
    ...
    BN_copy( q_minus_one , q );
    
    // Choose random secret k between 1 and q-1
    k = BN_myRandom(q);

    q_minus_one = BN_dup( q );
    ...
    // k^-1 mod q-1
    BN_mod_inverse(kInv, k, q_minus_one, ctx);
